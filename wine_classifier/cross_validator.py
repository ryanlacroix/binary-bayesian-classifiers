...
def k_fold_validation(full_set, datasets, fold_num, cl_type, show_trees = False):
	...
	for i in range(0, len(datasets)):
		for j in range(0, len(datasets)):
			#if (i, j) and (j, i) not in class_combos and i != j:
			#if i != j:
			class_combos.append((i,j))
	
	dep_trees = []
	for i in datasets:
		tree = dpnt.estimate_dep_tree(full_set, len(datasets[0][0]), show_trees)
		dep_trees.append(tree)

	# Depending on the dataset, it is sometimes beneficial to 
	# instead estimate one tree based on individual classes from the dataset.
	...
	# 		tree = dpnt.estimate_dep_tree(class_set, len(class_set[0]), False)
	# 		dep_trees.append(tree)

	# Create four identical tree structures based on total set
	all_conf_matrices = [[[[None] for x in range(len(datasets))] for y in range(len(datasets))] ]
...
def generate_avg_matrix(matrices):
	final_matrix = [[0 for x in range(len(matrices[0][0]))] for y in range(len(matrices[0][0]))]
	...
	for i in range(0, len(matrix[0])):
			for j in range(0, len(matrix[0])):
				final_matrix[i][j] = final_matrix[i][j] / len(matrices)
	return final_matrix
...
files = ['class1.csv', 'class2.csv', 'class3.csv']
...
k_fold_validation(full_set, class_datasets, 5, "dependent", False)
